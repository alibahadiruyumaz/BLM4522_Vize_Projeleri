...
-- Standart Temizlik İşlemi
DROP DATABASE IF EXISTS DB_Performans;
GO

-- Yeni Veritabanı İnşası
...
-- Tabloların Performans Testlerine Uygun Veri Tipleriyle Kurulması
...
-- İlişkisel Bütünlük (Foreign Key) Kısıtlamaları
...
-- Veri Büyütme (Scaling) İşlemi: İlişkisel Bütünlüğü Koruyarak Veri Çoğaltma
DECLARE @i INT = 1;

WHILE @i <= 5 
BEGIN
    -- Sipariş Kayıtlarının Çoğaltılması
    ...
    SELECT 
        order_id + '-' + CAST(@i AS NVARCHAR(10)), 
        customer_id, 
        order_status, 
        DATEADD(day, @i * 15, order_purchase_timestamp), 
        DATEADD(day, @i * 15, order_approved_at),
        DATEADD(day, @i * 15, order_delivered_carrier_date),
        DATEADD(day, @i * 15, order_delivered_customer_date),
        DATEADD(day, @i * 15, order_estimated_delivery_date)
    FROM Olist_Orders
    WHERE order_id NOT LIKE '%-%';

    -- Sipariş Detay Kayıtlarının Çoğaltılması
    ...
    SELECT 
        ...
        product_id, 
        seller_id, 
        DATEADD(day, @i * 15, shipping_limit_date), 
        price, 
        freight_value
