...
Projenin son aşamasında kurulan indeks mimarisinin gerçek sistem trafiği altındaki davranışı DMV (Dynamic Management Views) aracılığıyla doğrulanmış ve veritabanı erişim güvenliği yapılandırılmıştır.

**DMV ile İndeks Telemetrisi**

Sistem yeniden başlatıldıktan sonra Gün 3'te refaktör edilen SARGable sorgu çalıştırılmış ve ardından `sys.dm_db_index_usage_stats` görünümü sorgulanmıştır. Elde edilen telemetri verisinde `Olist_Orders` tablosuna ait `IX_Orders_Date_Status` indeksinin `user_seeks` değerinin 1 olarak kaydedildiği görülmüştür. Bu metrik, SQL Server Query Optimizer'ın artık tam tablo taraması (Table Scan) yerine oluşturulan kapsayan indeksi (Covering Index) kullandığını doğrulamaktadır. Gün 2'de ölçülen I/O maliyetindeki düşüşün geçici değil, mimari düzeyde kalıcı olduğu bu şekilde kanıtlanmıştır. İlgili sorgular `04_DMV_ve_Guvenlik.sql` dosyasında yer almaktadır.

**Rol Tabanlı Erişim Kontrolü (RBAC)**

Veritabanının yetkisiz müdahalelere karşı korunması amacıyla En Az Yetki Prensibi (Principle of Least Privilege) uygulanmıştır. Sunucu düzeyinde `AnalistLogin` girişi ve buna bağlı `AnalistUser` veritabanı kullanıcısı oluşturulmuştur. Bu kullanıcıya yalnızca `Olist_Orders` ve `Olist_Order_Items` tablolarında `SELECT` yetkisi tanımlanmıştır. Veri silme (`DELETE`), güncelleme (`UPDATE`) ve şema değiştirme (`DROP`/`ALTER`) işlemleri bu kullanıcı için tanımlanmamış olup varsayılan olarak kısıtlıdır.

---

**Sonuç**

Bu proje kapsamında Olist e-ticaret veri seti üzerinde dört aşamalı bir performans optimizasyon döngüsü uygulanmıştır. Gün 1'de ilişkisel mimari kurulmuş ve test ortamı hazırlanmıştır. Gün 2'de stres testi ile sistemin indekssiz durumdaki maliyeti ölçülmüş ve referans değerler belgelenmiştir. Gün 3'te Non-Clustered indeksler ve SARGable sorgu tasarımıyla disk I/O maliyeti %62, sorgu tamamlanma süresi %55 oranında düşürülmüştür. Gün 4'te DMV telemetrisiyle iyileştirmenin kalıcılığı doğrulanmış ve rol tabanlı erişim kontrolüyle veritabanı güvenliği sağlanmıştır.
